import { Token, tokenize, TokenType } from "../../cmd_lexer/lexer.js";
...
export class CMD extends App {
    ...
    static openApp(apartment = shared.activeApartment) {
        ...
        if (apartment == shared.activeApartment) {
            ...
            const mainScreen = appComponent.querySelector("#cmd") as HTMLElement;

            let editable = mainScreen.querySelector("[contentEditable]") as HTMLElement;
            mainScreen.addEventListener("click", ( e: MouseEvent ) => {
                editable = mainScreen.querySelector("[contentEditable]") as HTMLElement;
    ...
    async executeCommand(command: string) {
        ...
        if (mainScreen) mainScreen.scrollTop = mainScreen.scrollHeight;
    }

    removeEditable() {
        const contentEditable = this.window?.querySelectorAll<HTMLElement>("[contentEditable]");
        if (!contentEditable) return;
        
        contentEditable.forEach((element: HTMLElement) => {
    ...
    getStartLine() {
        ...
        const editable = mainScreen.querySelector("[contentEditable]") as HTMLElement;
        if (document.activeElement === document.body) editable.focus();
    }

    static getFlags(allowedFlags: { value: string, output: string }[], tokenized: Token[]) {
