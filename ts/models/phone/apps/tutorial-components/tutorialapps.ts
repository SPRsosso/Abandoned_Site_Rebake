export const tutorialAppsAndComponents = /*html*/`
    <h3>USB port connector</h3>
    <p>It is located on right side of your monitor, you can plug USB devices into it, such as Pendrive</p>

    <h3>Pendrive</h3>
    <p>It is used to crack into PCs, you can download additional program that will help you with hacking PCs</p>

    <h3>Console</h3>
    <p>You can do many things using this powerful app, you can change user name, break wifis, connect into wifis, open apps, delete or create files</p>

    <h3>File Explorer</h3>
    <p>It is used to explore documents or get inside folders</p>

    <h3>Calculator</h3>
    <p>It is used to calculate equations</p>

    <h3>Crack</h3>
    <p>It is used to crack computer's passwords</p>

    <h3>WiTracker</h3>
    <small>ID: WiTracker</small>
    <p>PC app that you can track Wifis nearby: getting their name, strength and connected PCs</p>

    <h3>Home Defence</h3>
    <small>ID: HomeDefence</small>
    <p>PC app that gets your apartment, checks your router stability and allows you to change routers remotely</p>

    <h3># Map</h3>
    <small>ID: HashMap</small>
    <p>Allows you to download program onto Pendrives</p>

    <h3>Personide</h3>
    <small>ID: Personide</small>
    <p>Allows access to person information</p>

    <h3>MessX</h3>
    <small>ID: MessX</small>
    <p>Small and compact communicator you can talk to with other people</p>
`;