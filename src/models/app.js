...
export class App {
    ...
    static closeApp(app, apartment = shared.activeApartment) {
        if (app)
            app.remove();
    ...
    static downloadApp(cmd, appName, system, apartment = shared.activeApartment) {
        const app = cmd.window;
        if (!apartment.router.connectedWifi) {
            if (app)
                Logger.error(app, "No Wifi connected!");
            return;
        }
    ...
    static closeAllApps(apartment = shared.activeApartment) {
