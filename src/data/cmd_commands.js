...
import { TokenType } from "../cmd_lexer/lexer.js";
...
const GLOBAL_COMMANDS = {
    ...
    dirlist: (tokenized, cmd) => {
        ...
        for (let i = 0; i < filesDir.length; i++) {
            ...
            Logger.log(cmd.window, file.name + " ");
        }
    },
    cdir: (tokenized, cmd) => {
        tokenized.shift();
    ...
    pcinfo: (tokenized, cmd) => {
        ...
        if (Apartment.activeApartment.router.connectedWifi)
            ...
            CMD.log(cmd.window, "Connected Wifi: Not connected");
    },
    ...
    changepassword: (tokenized, cmd) => {
        ...
        let password = "";
        if (tokenized.length > 0)
            password = tokenized.shift().value;
    ...
    codetable: async (tokenized, cmd, system) => {
        ...
        for (let i = 0; i < Wifi.possibleChars.length; i++) {
            ...
            if (i < Wifi.possibleChars.length - 1)
                CMD.inlog(cmd.window, slog, ", ");
        }
    },
    ...
    buy: (tokenized, cmd, system) => {
        ...
        Object.keys(items).forEach(item => {
            if (item.toLowerCase() === appName.toLowerCase())
                name = item;
        });
